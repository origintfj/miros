...
static int const _sd_init(uint32_t const spim_base_addr, unsigned const cphb_slow, unsigned const cphb_fast) {
    uint8_t r1;
    uint32_t vinfo, ocr;
...
sd_context_t *const sd_context_create(uint32_t const spim_base_addr, uint32_t const clk_freq_hz) {
    ...
    sd_context->clk_freq_hz = clk_freq_hz;
...
int const sd_init(sd_context_t *const sd_context) {
    return _sd_init(sd_context->spim_base_addr,
                    sd_context->clk_freq_hz / 333000 / 2,
                    sd_context->clk_freq_hz / 25000000 / 2);
}
...
size_t const sd_read(void *const buffer, size_t const size,
    ...
    uint32_t const spim_base_addr = sd_context->spim_base_addr;
    ...
    sd_send_dummy_bytes(spim_base_addr, 1);
    spim_cs_assert(spim_base_addr);
    spim_byte_exchange(spim_base_addr, 0x40 | 18);
    spim_byte_exchange(spim_base_addr, sector >> 24);
    spim_byte_exchange(spim_base_addr, sector >> 16);
    spim_byte_exchange(spim_base_addr, sector >>  8);
    spim_byte_exchange(spim_base_addr, sector >>  0);
    ...
    // throw away the unwanted bytes from the first block
    for (i = 0; i < offset; ++i) {
        spim_byte_exchange(spim_base_addr, 0xff);
    }

    // read out the wanted bytes
    for (i = 0; i < size * count; ++i) {
        ((uint8_t *const)buffer)[i] = spim_byte_exchange(spim_base_addr, 0xff);
    }

    spim_byte_exchange(spim_base_addr, 0x40 | 12);
    spim_byte_exchange(spim_base_addr, 0xff);
    spim_byte_exchange(spim_base_addr, 0xff);
    spim_byte_exchange(spim_base_addr, 0xff);
    spim_byte_exchange(spim_base_addr, 0xff);
...
/*
    // throw away the unwanted bytes from the last block
    for (i = 512; (offset + size * count) != 0 && i > (offset + size * count) & 0x1ff; --i) {
        spim_byte_exchange(spim_base_addr, 0xff);
    }
*/
    sd_context->cursor += (uint64_t const)(size * count);
